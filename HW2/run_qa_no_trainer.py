...
def save_prefixed_metrics(results, output_dir, epoch, file_name: str = "all_results.json", metric_key_prefix: str = "eval"):
    """
    ...
    with open(os.path.join(output_dir, f"all_results_{epoch}.json"), "w") as f:
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        data_files = {}
        if args.train_file is not None:
            data_files["train"] = Dataset.from_list(json.loads(Path(args.train_file).read_text()))
        if args.validation_file is not None:
            data_files["validation"] = Dataset.from_list(json.loads(Path(args.validation_file).read_text()))
        if args.test_file is not None:
    ...
    if args.config_name:
        # config = transformers.configuration_roberta.RobertaConfig
    ...
    def post_processing_test_function(examples, features, predictions, stage="eval"):
        ...
        if args.version_2_with_negative:
            ...
            formatted_predictions = [{"id": k, "prediction_text": v} for k, v in predictions.items()]
        print(pd.DataFrame(formatted_prediction))
        # references = [{"id": ex["id"], "answers": ex[answer_column_name]} for ex in examples]
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if epoch%1==0:
            ...
            prediction = post_processing_function(eval_examples, eval_dataset, outputs_numpy, stage = f"eval{epoch:03d}")
            ...
            if args.do_predict:
                ...
                outputs_numpy = (start_logits_concat, end_logits_concat)
                print(outputs_numpy[0].shape)
                prediction = post_processing_function(predict_examples, predict_dataset, outputs_numpy, stage=f"pred{epoch:03d}")
                predict_metric = metric.compute(predictions=prediction.predictions, references=prediction.label_ids)
        # logger.info(f"Predict metrics: {predict_metric}")

            if args.with_tracking:
                log = {
                    "QA" if args.version_2_with_negative else "squad": eval_metric,
                    "train_loss": total_loss.item() / len(train_dataloader),
                    "epoch": epoch,
                    "step": completed_steps,
                }
            if args.do_predict:
                log["QA_predict" if args.version_2_with_negative else "QA_predict"] = predict_metric
            ...
            if args.output_dir is not None:
                ...
                if accelerator.is_main_process:
                    ...
                    save_prefixed_metrics(eval_metric, args.output_dir, epoch)
    print("Total Loss:", total_loss_list)
